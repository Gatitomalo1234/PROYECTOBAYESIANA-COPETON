...
PARENT_DIR = os.path.dirname(BASE_DIR) # Gets the main 'PROYECTO BAYESIANA' folder
BIRDS_OBS_FILE = os.path.join(PARENT_DIR, 'ebird_raw_data', 'ebd_CO-CUN_smp_relJan-2026.txt')
BIRDS_SAMP_FILE = os.path.join(PARENT_DIR, 'ebird_raw_data', 'ebd_CO-CUN_smp_relJan-2026_sampling.txt')
...
OUTPUT_FILE = os.path.join(BASE_DIR, 'data', 'interim', 'birds_pollution_merged.csv')
...
def run_join():
    ...
    # This effectively keeps data for the 2021-2026 period where matches exist.
    pollutant_cols = list(p_rename.values())
    df_final = df_final.dropna(subset=pollutant_cols, how='all')
    print(f"Filtered to {len(df_final)} records within the 2021-2026 pollution data range.")
    ...
    print(f"while the pollution dataset now covers the augmented period 2021-2026.")
